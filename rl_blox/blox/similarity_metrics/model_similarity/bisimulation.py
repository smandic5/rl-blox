...
class Similarity(ABC):
    ...
    def _generate_cache_key(self, env1, env2, **kwargs) -> str:
        ...
        """
        desc1 = "".join(
            ["".join(row) for row in env1.unwrapped.desc.astype(str)]
        )
        desc2 = "".join(
            ["".join(row) for row in env2.unwrapped.desc.astype(str)]
        )
        ...
        for a in range(P.shape[1]):
            ...
            for s_next in range(P.shape[2]):
                if P[s, a, s_next] > 0:
                    ...
                    transition_edge_rewards[action_node, s_next] = R[
                        s, a, s_next
                    ]
...
class GraphSimilarity(Similarity):
    ...
    def _graph_distance(self, mdpg1, mdpg2):
        ...
        for it in range(self.max_iter):
            ...
            if (
                np.max(np.abs(S - S_old)) < self.tol
                and np.max(np.abs(A - A_old)) < self.tol
            ):
                break
        return S, A
                ...
                for j in range(n_j_states):
                    ...
                    for a_i in range(n_i_actions):
                        ...
                        for a_j in range(n_j_actions):
                            ...
                            vals.append(
                                (1 - self.c) * reward_diff + self.c * trans_diff
                            )
...
class RewardSimilarity(Similarity):
    ...
    def _reward_distance(self, P1, R1, P2, R2):
        ...
        assert (
            R1_exp.shape == R2_exp.shape
        ), "Reward matrices must match in shape"
        diff = R1_exp - R2_exp
        mse = np.mean(diff**2)
        return mse
